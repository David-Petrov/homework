/*
Даден е израз, формулиран чрез числа, знаци за събиране и умножение и скоби. Да
се напише функция, която пресмята стойността му.
Забележка: нямаме приоритет на действията, т.е. горния израз е равен на 5*(5*5)+7
Пример:
Вход:
2+3*((1+4)*5)+7
Изход:
